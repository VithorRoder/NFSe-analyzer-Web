...
<style>
:root{font:15px system-ui,sans-serif;color:#1b2741;background:#f5f7ff}*{box-sizing:border-box}body{margin:0}button,input,select{font:inherit}button{cursor:pointer}main{max-width:1440px;margin:auto;padding:36px 28px 70px}.eyebrow{color:#526de1;font-size:12px;font-weight:800;letter-spacing:2px}.heading{display:flex;align-items:end;justify-content:space-between;gap:20px;flex-wrap:wrap}h1{font-size:clamp(28px,4vw,42px);letter-spacing:-1px;margin:7px 0}.sub{color:#6b7891;margin:0}.summary{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:15px;margin:30px 0}.card,.panel{background:#fff;border:1px solid #e1e7f3;border-radius:17px;box-shadow:0 8px 30px #273b6810}.card{padding:20px}.card span{color:#75829b;font-size:13px}.card strong{display:block;font-size:clamp(20px,2vw,28px);margin-top:8px;overflow-wrap:anywhere}.card small{color:#8c98ad}.valid-number{color:#228064}.filters{display:flex;gap:12px;flex-wrap:wrap;align-items:end;padding:20px;margin-bottom:18px}.filters label{display:grid;gap:6px;font-weight:700;font-size:12px;color:#65728a}.filters input,.filters select{height:42px;border:1px solid #dce3f1;border-radius:9px;background:#fff;color:#1b2741;padding:0 12px}.filters .search{flex:2;min-width:240px}.filters .search input{width:100%}.filters .date{min-width:155px}.filters .date input{width:100%}.panel{overflow:hidden}.table-wrap{overflow:auto;max-height:660px}table{width:100%;border-collapse:collapse;min-width:990px}th,td{padding:14px 16px;text-align:left;border-bottom:1px solid #ebeff6}th{position:sticky;top:0;background:#f2f5fc;color:#697791;text-transform:uppercase;font-size:11px;letter-spacing:.6px;z-index:1}tbody tr{cursor:pointer}tbody tr:hover,tbody tr:focus{background:#eef2ff;outline:0}td{font-size:13px}td.money{font-weight:700;white-space:nowrap}td.muted{color:#8a96aa}.pill{display:inline-block;border-radius:999px;padding:5px 10px;font-size:11px;font-weight:700;white-space:nowrap}.pill.valid{background:#e7f6ee;color:#25805d}.pill.cancelled{background:#fff0e7;color:#ae6135}.pill.substituted{background:#f0ebfb;color:#7656ad}.pager{display:flex;justify-content:space-between;align-items:center;gap:15px;flex-wrap:wrap;padding:17px 20px;color:#65728a}.pager-controls{display:flex;align-items:center;gap:12px}.pager button,.close{border:1px solid #dce3f1;background:#fff;color:#425fc9;border-radius:9px;padding:9px 14px;font-weight:700}.pager button:disabled{opacity:.45;cursor:default}.empty{padding:30px;color:#7b879c;text-align:center}dialog{border:0;border-radius:18px;padding:0;width:min(900px,95vw);max-height:90vh;overflow:auto;box-shadow:0 22px 70px #14223b55}dialog::backdrop{background:#14223b99}.detail-head{position:sticky;top:0;z-index:2;display:flex;justify-content:space-between;align-items:center;gap:15px;background:#fff;border-bottom:1px solid #e6eaf3;padding:22px 28px}.detail-head h2{margin:3px 0;font-size:23px}.detail-body{padding:24px 28px 30px}.group{margin-bottom:27px}.group h3{font-size:15px;margin:0 0 13px;color:#344765}.fields{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.field{padding:12px 14px;background:#f7f9fd;border:1px solid #edf1f7;border-radius:10px;min-width:0}.field dt{font-size:11px;color:#74819a;font-weight:700;margin-bottom:5px}.field dd{margin:0;overflow-wrap:anywhere;font-size:13px}.raw{display:grid;gap:7px}.raw .field{display:grid;grid-template-columns:minmax(180px,1fr) minmax(0,1fr);gap:10px}.notice{font-size:12px;color:#71809a;line-height:1.5}.hidden{display:none!important}@media(max-width:800px){.summary{grid-template-columns:repeat(2,minmax(0,1fr))}.fields{grid-template-columns:1fr}}@media(max-width:500px){main{padding:25px 14px}.summary{gap:9px}.card{padding:15px}.filters{padding:15px}.filters label,.filters .search{min-width:100%}.detail-head,.detail-body{padding:18px}.raw .field{grid-template-columns:1fr}}
...
function render(){const q=$('search').value.trim().toLocaleLowerCase('pt-BR'),status=$('status').value,direction=$('direction').value,from=$('from').value,to=$('to').value,sort=$('sort').value;const list=data.filter(n=>(!status||n.status===status)&&(!direction||n.direction===direction)&&(!from||n.issue>=from)&&(!to||n.issue<=to)&&(!q||[n.key,n.number,n.issuer,n.issuerId,n.recipient,n.recipientId,n.service].some(v=>String(v||'').toLocaleLowerCase('pt-BR').includes(q))));list.sort((a,b)=>sort==='oldest'?a.issue.localeCompare(b.issue):sort==='highest'?b.serviceValue-a.serviceValue:sort==='lowest'?a.serviceValue-b.serviceValue:b.issue.localeCompare(a.issue));const pages=Math.max(1,Math.ceil(list.length/pageSize));page=Math.min(Math.max(0,page),pages-1);const body=$('rows');body.replaceChildren();for(const note of list.slice(page*pageSize,(page+1)*pageSize)){const tr=document.createElement('tr');tr.tabIndex=0;for(const [i,item] of [note.number||'—',date(note.issue),note.issuer||note.issuerId||'—',note.recipient||note.recipientId||'—',note.service||'—',money(note.serviceValue),note.status].entries()){const td=document.createElement('td');if(i===5)td.className='money';if(i===6){const badge=document.createElement('span');badge.className='pill '+(item==='Válida'?'valid':item==='Cancelada'?'cancelled':'substituted');badge.textContent=item;td.append(badge)}else td.textContent=item;tr.append(td)}tr.addEventListener('click',()=>show(note));tr.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();show(note)}});body.append(tr)}$('empty').classList.toggle('hidden',list.length>0);$('count').textContent=(list.length?page*pageSize+1:0)+'–'+Math.min(list.length,(page+1)*pageSize)+' de '+list.length+' resultado(s)';$('page').textContent='Página '+(page+1)+' de '+pages;$('prev').disabled=page===0;$('next').disabled=page>=pages-1}
const valid=data.filter(n=>n.status==='Válida');$('total-count').textContent=data.length;$('valid-count').textContent=valid.length;$('valid-total').textContent=money(valid.reduce((sum,n)=>sum+(n.serviceValue||0),0));$('valid-taxes').textContent=money(valid.reduce((sum,n)=>sum+(n.pisValue||0),0))+' / '+money(valid.reduce((sum,n)=>sum+(n.cofinsValue||0),0));for(const id of ['search','status','direction','from','to','sort'])$(id).addEventListener(id==='search'?'input':'change',()=>{page=0;render()});$('prev').addEventListener('click',()=>{page--;render()});$('next').addEventListener('click',()=>{page++;render()});$('close').addEventListener('click',()=>$('detail').close());$('detail').addEventListener('click',e=>{if(e.target===$('detail'))$('detail').close()});render();
</script></body></html>`;
}
