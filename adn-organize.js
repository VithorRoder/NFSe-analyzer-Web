...
<div class="summary" aria-label="Resumo das notas filtradas">
  <div class="card"><span>Notas no filtro</span><strong id="total-count">0</strong><small>Inclui todas as situações</small></div>
  <div class="card"><span>Notas válidas</span><strong id="valid-count">0</strong><small>Canceladas e substituídas à parte</small></div>
  <div class="card"><span>Valor dos serviços válidos</span><strong class="valid-number" id="valid-total">—</strong><small>Somente notas válidas</small></div>
</div>
<section class="metric-section" aria-labelledby="tax-title">
  <div class="metric-heading"><h2 id="tax-title">Tributos das notas válidas</h2><p>Valores informados nos XMLs, sem apuração adicional.</p></div>
  <div class="tax-grid">
    <div class="card"><span>ISS</span><strong id="iss-total">—</strong><small id="iss-coverage"></small></div>
    <div class="card"><span>PIS</span><strong id="pis-total">—</strong><small id="pis-coverage"></small></div>
    <div class="card"><span>Cofins</span><strong id="cofins-total">—</strong><small id="cofins-coverage"></small></div>
  </div>
</section>
<section class="metric-section" aria-labelledby="retention-title">
  <div class="metric-heading"><h2 id="retention-title">Retenções informadas</h2><p>Totais de notas válidas; o total retido pode incluir outros componentes.</p></div>
  <div class="retention-grid">
    <div class="card"><span>Total retido</span><strong id="retained-total">—</strong><small id="retained-coverage"></small></div>
    <div class="card"><span>IRRF retido</span><strong id="irrf-total">—</strong><small id="irrf-coverage"></small></div>
    <div class="card"><span>CSLL retida</span><strong id="csll-total">—</strong><small id="csll-coverage"></small></div>
    <div class="card"><span>Previdência retida</span><strong id="social-security-total">—</strong><small id="social-security-coverage"></small></div>
  </div>
</section>
...
function updateMetric(valid, field, totalId, coverageId){
  const informed=valid.filter(n=>n[field]!=null);
  $(totalId).textContent=informed.length?money(informed.reduce((sum,n)=>sum+n[field],0)):valid.length?'Não informado':money(0);
  $(coverageId).textContent=valid.length?informed.length+' de '+valid.length+' nota(s) com valor informado':'Sem notas válidas no filtro';
}
function updateSummary(list){
  const valid=list.filter(n=>n.status==='Válida');
  $('total-count').textContent=list.length;
  $('valid-count').textContent=valid.length;
  $('valid-total').textContent=money(valid.reduce((sum,n)=>sum+(n.serviceValue||0),0));
  for(const [field,totalId,coverageId] of [
    ['issValue','iss-total','iss-coverage'],['pisValue','pis-total','pis-coverage'],
    ['cofinsValue','cofins-total','cofins-coverage'],['totalRetained','retained-total','retained-coverage'],
    ['irrfRetained','irrf-total','irrf-coverage'],['csllRetained','csll-total','csll-coverage'],
    ['socialSecurityRetained','social-security-total','social-security-coverage']
  ]) updateMetric(valid,field,totalId,coverageId);
}
